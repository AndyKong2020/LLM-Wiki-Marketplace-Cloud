...
llm-wiki-cloud-backflow
    |
    +--> 1. 轨迹归档：运行内置 session-extractor，并把本次任务整理成本地目录 .llm-wiki/backflow/<task-slug>/
...
- 内置 session-extractor 生成的当前会话轨迹：`workspace/session-extractor/`
- 既有 `.agents-log/summary/<timestamp>/` 目录（如果存在，仅作为 legacy 附件）
...
- `.git/`、`.idea/`、`.vscode/`、`.llm-wiki/backflow/`、`.session-extractor/`（避免递归归档）
- 若有 legacy `.agents-log`，默认不上传 `meta/` 目录

如果被排除的材料有证据价值，在 `<task-slug>.md` 的 `Notes` 中写清原路径、原因、摘要和可访问位置；不要把大文件硬塞进上传。

### 1.3 生成并纳入 session 轨迹

backflow 必须在归档阶段自己生成当前会话轨迹，不依赖外部 `.agents-log` 先运行。

先定位本 skill 内置的 session-extractor 脚本。脚本随 `llm-wiki-cloud-backflow` 一起发布，路径通常是当前 skill 目录下的 `scripts/extract.py`。如果当前客户端没有直接暴露 skill 目录，就用下面的 Bash 片段查找：
...
names = [
    "llm-wiki-cloud-backflow/scripts/extract.py",
    "plugins/llm-wiki-client-opencode/skills/llm-wiki-cloud-backflow/scripts/extract.py",
]
env_file = os.environ.get("LLM_WIKI_SESSION_EXTRACTOR")
if env_file and Path(env_file).expanduser().is_file():
    print(Path(env_file).expanduser())
    raise SystemExit(0)

search_roots = []
for key in ["CLAUDE_PLUGIN_ROOT", "CODEX_PLUGIN_ROOT", "OPENCODE_PLUGIN_ROOT"]:
...
direct_roots = search_roots + [Path.cwd(), *list(Path.cwd().parents)[:4]]
...
for root in search_roots:
    if not root.exists():
        continue
    for candidate in root.rglob("skills/llm-wiki-cloud-backflow/scripts/extract.py"):
        if candidate.is_file():
            print(candidate)
            raise SystemExit(0)

raise SystemExit("cannot locate bundled session-extractor scripts/extract.py")
PY
)"
```

然后显式传入当前平台，输出到 archive 内：

```bash
session_trace_dir="${archive_root}/workspace/session-extractor"
mkdir -p "${session_trace_dir}"

python3 "${session_extractor_script}" \
  --platform opencode \
  --mode structured \
  --out "${session_trace_dir}" \
  --cwd "${workspace}"

rm -rf "${session_trace_dir}/meta/locks"
```

处理规则：

- 脚本成功：把 `workspace/session-extractor/summary/` 和 `workspace/session-extractor/meta/` 一起保留在 archive 中，但删除 `workspace/session-extractor/meta/locks/`，它只是幂等锁文件，不是轨迹证据。
- 脚本失败：不要阻断归档；把 stderr/stdout 摘要写入 `<task-slug>.md` 的 `Notes`，并在最终汇报中说明 `session_trace_status=failed`。
- 如果已有 legacy `.agents-log/summary/` 且与当前任务相关，可以额外复制到 `workspace/agents-log/summary/`；但不得把它作为唯一轨迹来源。
...
└── workspace/              # 任务现场材料
    ...
    ├── session-extractor/  # backflow 内置脚本生成的当前会话轨迹
    ├── agents-log/         # legacy，如已有相关 .agents-log summary
    └── ...
```
...
- 按第 1.3 节运行内置 session-extractor，把结果写入 `workspace/session-extractor/`。
- 如有相关 legacy agents-log summary，可额外复制到 `workspace/agents-log/summary/`。
...
`workspace/session-extractor/summary/opencode/<timestamp>/summary.md` 路径（由 backflow 内置 session-extractor 生成）

## Wiki Usage History

`workspace/wiki_usage.md` 路径（如有）

## Legacy Agents Log Summary

`workspace/agents-log/summary/<timestamp>/summary.md` 路径（如有 legacy .agents-log）
...
- 记录 session-extractor 执行状态、生成的 summary 路径；如果失败，写明失败原因和是否纳入 legacy `.agents-log`
...
- `session_trace_status=ok | failed`，以及 `workspace/session-extractor/summary/.../summary.md` 路径；如果失败，说明原因
- 纳入了哪些 legacy `.agents-log/summary/<timestamp>/`；如果没有找到相关 summary，明确说明已跳过
- 排除了哪些重要文件以及原因
- 即将作为上传 `slug` 的值
