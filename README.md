...
backflow 会运行内置 session-extractor 生成当前会话轨迹。若用户确认且配置了
...
与 MCP 配置，并复制 backflow 内置 session-extractor 脚本。维护生成产物时不要直接
手改 `plugins/*/skills/**/SKILL.md`、`plugins/*/skills/**/scripts/**` 或 manifest；
应修改源模板/脚本源后重新同步。
